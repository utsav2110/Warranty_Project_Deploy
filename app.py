...
def get_user_email(user_id):
    """Fetch user email from database using user_id"""
    conn = get_conn()
    cur = conn.cursor()
...
def send_email(to_email, subject, body):
    ...
    message.attach(MIMEText(body, "plain"))  # Plain text version
...
def login_user(username, password):
    try:
        ...
        st.session_state["user_id"] = row[0]  # Store user_id in session
